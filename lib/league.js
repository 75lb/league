...
League.prototype.addResult = function(result){
    ...
    if (Array.isArray(result)){
        ...
        if (table.previous){
            l("%s  %s  %s", homeTeam.team, homeTeam.position, table.previous.getTeam(homeTeam.id).position);
        }
        if (!table.previous){
            homeTeam.move = "-";
        } else if (homeTeam.position < table.previous.getTeam(homeTeam.id).position){
            homeTeam.move = "up";
        } else if (homeTeam.position == table.previous.getTeam(homeTeam.id).position){
            homeTeam.move = "-";
        } else {
        ...
        if (!table.previous){
            awayTeam.move = "-";
        } else if (awayTeam.position < table.previous.getTeam(awayTeam.id).position){
            awayTeam.move = "up";
        } else if (awayTeam.position == table.previous.getTeam(awayTeam.id).position){
            awayTeam.move = "-";
        } else {
